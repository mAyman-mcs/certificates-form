...
function parseWorkbook(filePath) {
  return parseWorkbookData(XLSX.readFile(filePath, { cellDates: true }));
}

/** Same as parseWorkbook, but for an already-loaded buffer (e.g. fetched from S3). */
function parseWorkbookBuffer(buffer) {
  return parseWorkbookData(XLSX.read(buffer, { type: 'buffer', cellDates: true }));
}
...
module.exports = { parseWorkbook, parseWorkbookBuffer, parseExpirationRaw, computeStatus };
